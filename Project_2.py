...
# multiply(user_list)
# Заменил цикл на генератор списков и добавил тернарный оператор

def multiply(list):
    result_len = len(list) // 2 + 1 if len(list) % 2 == 1 else len(list) // 2
    result_list = [list[i] * list[-(i + 1)] for i in range(result_len)]
    print(f"Результат {result_list}")
